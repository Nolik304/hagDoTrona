  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="panel relative overflow-hidden">
        ...
        <div className="relative h-52 grid place-items-center">
          {e && (
            <>
      ...
      <div className="grid grid-cols-4 gap-2">
        {skills.map(sk => {
          ...
          const ready = cd <= 0 && !locked && !!e && !B.paused;
          return (
            <button key={sk.id} onClick={() => d({ type: "CAST", id: sk.id })} disabled={!ready}
              className="relative btn btn-dark py-2.5 px-1 overflow-hidden h-[74px] flex flex-col items-center justify-center gap-0.5"
              style={ready ? { borderColor: "#f0b42966", boxShadow: "0 4px 0 #0c1017, 0 0 12px rgba(240,180,41,0.15)" } : undefined}>
              <Icon n={sk.icon} className="w-6 h-6 text-gold2" />
              <span className="text-[9px] leading-tight text-fog font-body font-semibold text-center px-0.5">{sk.name}</span>
              <span className="text-[9px] text-dim font-body">ур. {lvl}</span>
              {locked && (
